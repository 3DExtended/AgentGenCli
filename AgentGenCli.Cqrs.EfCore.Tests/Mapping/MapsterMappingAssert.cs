...
using AgentGenCli.Cqrs.EfCore.Abstractions;
using MapsterMapper;
...
public static class MapsterMappingAssert
{
    ...
    )
    {
        ...
        var source = Activator.CreateInstance<TSource>()
            ?? throw new InvalidOperationException($"Could not create an instance of {typeof(TSource).Name}.");
    ...
    private static bool IsIdentifierType(Type type)
    {
        ...
        {
            if (current.IsGenericType && current.GetGenericTypeDefinition() == typeof(Identifier<,>))
            {
                return true;
            }
        }
    ...
    private static void PopulateInstance(object instance, HashSet<(Type Type, string Name)> excluded)
    {
        ...
        for (var type = identifierType; type is not null; type = type.BaseType)
        {
            ...
            if (fromMethod is not null)
            {
                return fromMethod.Invoke(null, [value])
                    ?? throw new InvalidOperationException($"From(Guid) on '{identifierType.Name}' returned null.");
            }
        }

        throw new InvalidOperationException($"Identifier type '{identifierType.Name}' is missing a From(Guid) method.");
    }
    ...
    private static bool ValuesMatch(object? expected, object? actual)
    {
        ...
        {
            var expectedId = expected.GetType().GetProperty(nameof(IIdentifier<Guid>.Value))!.GetValue(expected);
            var actualId = actual.GetType().GetProperty(nameof(IIdentifier<Guid>.Value))!.GetValue(actual);
        ...
        {
            return expectedEnumerable.Cast<object?>().SequenceEqual(actualEnumerable.Cast<object?>());
        }
